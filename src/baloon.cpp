...
double baloon_gravity = 0.5, tim = (double) 1/60;

Baloon::Baloon(float x, float y, color_t color) {
	...
	for(int i=0; i<9000;)
	{
		...
		vertex_buffer_data[i++] = 0.8 * cos(arg);
		vertex_buffer_data[i++] = 0.8 * sin(arg);
		vertex_buffer_data[i++] = 0.0f;
...
void Baloon::set_position(float x, float y) {
    this->position = glm::vec3(x, y, 0);
}
...
void Baloon::tick() {
    ...
    if (this->position.y < 6 or this -> speed_y < 0)
        this->position.y += this->speed_y;

    if (this->position.y > -ground)
        this->speed_y = this->speed_y - baloon_gravity * tim;
